...
fn main() {
    ...
    println!("\nConnect to server by visiting: https://{}:3000\n CTRL+CLICK",print_url);
    ...
    println!("\nSuccessful Connection to ({})", config.host_ip);
                    ...
                    if let Err(e) = file.write_all(&data) {
                        ...
                        if expected_file_size > 0 {
                            if last_print.elapsed().as_millis() > 200 {
                                ...
                                let size_match = expected_file_size as f64;
                                 if ( size_match/(1000.0*1000.0*1000.0))>1.0{
                                        print!("\rCollecting data:  {:.2} Gigabytes {:.2}%", size_match/(1024.0*1024.0*1024.0),percentage);
                                    }else if (size_match /(1000.0*1000.0))>1.0{
                                        print!("\rCollecting data:  {:.2} Megabytes {:.2}%", size_match/(1024.0*1024.0),percentage);
                                    }else if (size_match /(1000.0))>1.0{
                                        print!("\rCollecting data:  {:.2} Kilobytes {:.2}%", size_match/1024.0,percentage);
                                    }
